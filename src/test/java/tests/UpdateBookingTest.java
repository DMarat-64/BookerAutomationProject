...
public class UpdateBookingTest {
    ...
    public void testUpdateBooking() throws JsonProcessingException {
        ...
        CreateBooking updateData = new CreateBooking();
        updateData.setFirstname("Jonathan");
        updateData.setLastname("Falker");
        updateData.setTotalprice(500);
        updateData.setDepositpaid(true);
        updateData.setBookingdates(new BookingDates("2025-04-08", "2025-04-10"));
        updateData.setAdditionalneeds("Breakfast");

        String updateBody = objectMapper.writeValueAsString(updateData);
        Response updateBookingId = apiClient.putBookingId(updateBody, bookingId);

        String responseBodyFromPatch = updateBookingId.getBody().asString();
        CreateBooking updateBooking = objectMapper.readValue(responseBodyFromPatch, CreateBooking.class);

        assertThat(updateBookingId.getStatusCode()).isEqualTo(200);
